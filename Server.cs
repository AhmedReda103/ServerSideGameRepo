    ...
    internal class Server
    {
        ...
        private void handleResponseFromClient(string message , ClientHandler client)
        {
            ...
            if (data[0] == Utilities.LOGINREQUEST)
            {
                ...
                Debug.WriteLine(" iam watcherClient id from login request =  " + client.id);  
            ...
            {
                Debug.WriteLine(" rooms that created and send to clients ************ " + Utilities.ROOMJOINEDSUCESSFULLY + ";" + prepareRooms(rooms));
            ...
            else if (data[0] ==Utilities.CONFIGUREROOM)
            {
                ...
                Debug.WriteLine("in configure room " + roomData[0] + " room id = " + roomData[1]);           
                ...
                Debug.WriteLine($"iam data of zero {data[0]}, and i am dtat of one {data[1]}");
            ...
            else if (data[0] == Utilities.STARTGAME)
            {
                ...
                Debug.WriteLine(" player2 in response start game " + player2Id);
                Debug.WriteLine(" rooom id in start game that i save in dictionary game " + roomId);

                list_rooms[roomId].player2 = list_clients[player2Id];

                Room startedRoom = list_rooms[roomId];

                Debug.WriteLine(" startedRoom.player2 " + startedRoom.player2.id);

                Game game = new Game(startedRoom);

                lock (_lock)
                {
                    Debug.WriteLine($"Room Id Game that added to list_games========  {roomId}");
                    list_games.Add(roomId , game);
                    updateWatcherRoomsView();
                }

                startGameForTwoPlayers(roomId ,game);
                //startGameForTwoPlayers2(roomId ,startedRoom);
            }
        ...
        private void sendRequestToPlayer1(string[] response )
        {
            ...
            Debug.WriteLine($" room that player two want to join = {roomId}");
            ...
            if (list_rooms.ContainsKey(int.Parse(roomId)))
            {
                ...
                //list_rooms[parseRoomId].player2 = list_clients[int.Parse(player2Id)];
                
                Debug.WriteLine("iam in send request to player 1 ");
                Debug.WriteLine("iam in send request player 1 = " + list_rooms[parseRoomId]);
                Debug.WriteLine($"Item with key {roomId} removed successfully.");
            }
            else
            {
                Debug.WriteLine($"room with key {roomId} not found in the dictionary.");
            }
        ...
        private void deleteRoomFromRoomList(string deletedId)
        {
            lock (lockObject)
            {
                foreach(Room room in list_rooms.Values) {
                    ...
                    {
                        Debug.WriteLine("innnnnnnnnnnnnnnnnnnnnnn");
        ...
        {   string ans = string.Empty;
           ...
           /* if (list_rooms.Count > 0)
            {
                foreach(var room in list_rooms.Values)
                {
                    ans += room.ToString();
                    ans += ";";
                }
        ...
        void OnClientConnected(IAsyncResult ar)
        {
            try
            {
                ...
                Debug.WriteLine("watcherClient connected to server = " + clientID);

                Debug.WriteLine(" clientID of watcherClient ref " + clientRef.Count + " name " +  clientRef[0].clientName + " id " +  clientRef[0].id);
                clientID++; 

      
                //Task.Run(() => sendToClient(clientHand));
